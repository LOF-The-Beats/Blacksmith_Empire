...
void Surface::Print(const char* s, int x1, int y1, uint c, float scale)
{
	...
	for (int i = 0; i < len; ++i)
	{
		...
		int pos = (ch >= 'A' && ch <= 'Z')
			? transl[(unsigned short)(ch - ('A' - 'a'))]
			: transl[ch];

		// compute scaled size using ceil to avoid truncation
		int scaledW = (int)ceil(charWidth * scale);
		int scaledH = (int)ceil(charHeight * scale);

		for (int py = 0; py < scaledH; ++py)
		{
			float fy = py / scale;
			int v = (int)fy;
			if (v >= charHeight) continue;
			const char* row = font[pos][v];

			for (int px = 0; px < scaledW; ++px)
			{
				float fx = px / scale;
				int h = (int)fx;
				if (h >= charWidth) continue;
